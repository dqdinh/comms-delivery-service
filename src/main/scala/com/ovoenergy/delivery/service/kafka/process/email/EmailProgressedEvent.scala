...
object EmailProgressedEvent extends LoggingWithMDC {
  ...
  def send(publishEvent: (EmailProgressed) => Future[RecordMetadata])(comm: ComposedEvent, gatewayComm: GatewayComm)(
    ...
    publishEvent(event).map(record => {
      logInfo(
        ...
        s"Publishing EmailProgressed event: ${event.status} - ${event.gateway} - ${event.gatewayMessageId} - ${record.partition}/${record.offset}"
      )
      record
    })
  }
